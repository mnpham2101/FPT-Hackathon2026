# Phase 5 IVI–ADA troubleshooting — v1

> **Finding: the R4 warning never reaches the AAOS guest — ADA → IVI traffic dies before the wire** (§ ADA → IVI hop investigation below). The app's own receive path is proven good by loopback injection; the loss is between the ADA node's socket and the Room bridge.

Evidence record for the Phase 5 in-Room IVI subtasks of [phase5_minh_tasks.md](../phase5_minh_tasks.md) group 5.9, snapshotted onto this branch from the run record `plans/doc/phase5-ivi-run.md` on `main` (which stays the canonical, updated copy). Each section carries one subtask's recorded outputs; the procedure the outputs came from is [deploy-ivi-hmi-walkthrough.md](../../requirements/car-sky-guide/deploy-ivi-hmi-walkthrough.md), cited by section.
