...
**Full decomposition:** [phase4_tasks.md](phase4_tasks.md) — 8 task groups, 24 subtasks (15 agent incl. 1 optional, 1 [[car-sky]], 4 user-manual, 4 gated on user ratification). **Unaffected by the update** — no subtask changes, no ID moves.

**Input.** Phase 2 complete, with the CRA seam frozen; a live or mocked R2 source (`tools/mock_v2x_sender.py`, or the deployed Phase 1 chain for live evidence). **Phase 3 is not an input** — the detector's own-sensor B track is interchangeable with the Phase 2 fixture everywhere except the live deployed run in group 4.6.
...
**Task groups and IDs.** 4.1 `15.4.1.1` · `14.4.1.2` · `14.4.1.3` — 4.2 `15.4.2.1` … `15.4.2.4` — 4.3 `18.4.3.1` … `18.4.3.3` — 4.4 `6.4.4.1` · `6.4.4.2` — 4.5 `15.4.5.1` — **4.6 (deploy + live evidence)** `5.4.6.1` · `5.4.6.2` · `13.4.6.3` · `18.4.6.4` · `15.4.6.5` — 4.7 (gated) `4.4.7.1` … `4.4.7.4` — 4.8 (last) `20.4.8.1` · `21.4.8.2`.

**Suggested branch:** `feat/phase4-ada-fusion-warning`.
...
- **Three shared platform constraints** are the real contention, and they are scheduling constraints rather than dependencies: the **2-concurrent-Room deployment quota** (Phase 3's `5.3.6.2`, Phase 4's group 4.6 and Phase 5's groups 5.8/5.9 each need a Room — serialize the *deploys*, not the development); the single registry namespace; and the CI runner.
...
| 5 | **Deployment quota — 2 concurrent Rooms** against three groups that each need one (`5.3.6.2`, group 4.6, groups 5.8/5.9). Serialize the deploys and tear each Room down; `4.5.9.4` already ends with a teardown | project-planner (scheduling) |
