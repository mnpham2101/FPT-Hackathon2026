...
| 10 | ~~**`tools/ada-bench/` is a fifth container build context outside the four node folders**~~ — **CLOSED 2026-08-03.** [[project-architecture]] added [node-code-layout.md § `tools/` — test equipment and ECU mocks](../.claude/rules/node-code-layout.md#tools--test-equipment-and-ecu-mocks) (commit `99ae675`), naming `tools/ada-bench/` beside `tools/netcheck/` and `tools/comms_check/` and applying the four node folders' build rules to it. The cross-node placement question is settled for every future mock, not only this one | **closed** — [[project-architecture]] `99ae675` |
| 11 | ~~**[node-ada-ecu.md](../requirements/car-sky-guide/node-ada-ecu.md) contradicts the walkthrough on three node facts**~~ — **CLOSED 2026-08-03.** [[project-architecture]] corrected `command`, `capabilities` and the registry host (commit `62b69a2`), satisfying [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these)'s before-any-ADA-deploy condition. **Both phases are unblocked**: `5.3.6.2` and `5.4.10.5` no longer depend on Phase 2 `5.2.9.4`, which keeps only its non-blocking env-table half. `capabilities: ["NET_RAW"]` is recorded there as unconditional on the ADA node | **closed** — [[project-architecture]] `62b69a2` |
...
*Updated 2026-08-03 (later the same day) by project-planner on three user rulings. The isolated Room's images are now stated at index level — **three nodes, two images, both built and pushed by GitHub Actions, none by hand**, with `m1-ada-bench:latest` deployed twice under different `ROLE` values. **Open items 10 and 11 are closed** against [[project-architecture]]'s commits `99ae675` (`tools/` sanctioned in `node-code-layout.md`) and `62b69a2` (`node-ada-ecu.md` corrected, `NET_RAW` recorded as unconditional), which unblocks `5.3.6.2` and `5.4.10.5` from Phase 2 `5.2.9.4`. No ID added, removed or renumbered.*
