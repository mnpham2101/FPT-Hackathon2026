...
**Full decomposition:** [phase4_tasks.md](phase4_tasks.md) — **10 task groups, 37 subtasks** (20 agent incl. 1 optional, 9 [[car-sky]], 8 human). Groups 4.9–4.11 are stage 2 of [walkthrough-driven-delivery.md](../.claude/rules/walkthrough-driven-delivery.md) over [deploy-ada-ecu-walkthrough.md](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md).
...
**Then a second Room, and it is what makes Phase 4's boxes live.** Group 4.12's *alternative test* (`13.4.12.1`, one human subtask) keeps the ADA node byte-identical and replaces the **upstream** mock with the real bench Scenario Player and V2X ECU, leaving only the sink mocked. Relayed C then originates in a real scenario and arrives as a real decoded CPM — the *"with bench scenarios live"* wording of the acceptance boxes, met without any IVI app. It creates no new checks: groups 4.10–4.11's [[car-sky]] subtasks re-run against it under their existing briefs.

**Only the rendering half waits on Phase 5.** The full 5-node system test — the God view drawing ghost C from live relayed data — is [phase5_minh_tasks.md group 5.10](phase5_minh_tasks.md#task-group-510--system-verification-test-serves-r4-r5-r6-r16-r17-r18-r19), scheduled once and re-recorded in Phase 6. Phase 4's former group 4.6 duplicated it and is retired; its IDs (`5.4.6.1`, `5.4.6.2`, `13.4.6.3`, `18.4.6.4`) are not reused, and `15.4.6.5` survives alone as the ADA→IVI pcap. **For this cross-node plan the consequence that matters: no Phase 4 acceptance box waits on Phase 5's app, Phase 5's isolated IVI test (group 5.9) does not wait on the real ADA node, and the one run that genuinely needs both nodes finished is scheduled once rather than twice.**
...
**Task groups and IDs.** 4.1 `15.4.1.1` · `14.4.1.2` · `14.4.1.3` — 4.2 `15.4.2.1` … `15.4.2.4` — 4.3 `18.4.3.1` … `18.4.3.3` — 4.4 `6.4.4.1` · `6.4.4.2` — 4.5 `15.4.5.1` — **4.6 (ADA→IVI pcap)** `15.4.6.5` — **4.12 (alternative test — real bench and V2X ECU upstream)** `13.4.12.1` — **4.9 (bench image, CI lanes, blueprint reference)** `5.4.9.1` · `2.4.9.2` · `4.4.9.3` · `5.4.9.4` · `5.4.9.5` · `5.4.9.6` · `2.4.9.7` — **4.10 (create and deploy the isolated Room)** `5.4.10.1` … `5.4.10.8` — **4.11 (the three checks, negative case, teardown)** `18.4.11.1` · `2.4.11.2` · `13.4.11.3` · `15.4.11.4` · `13.4.11.5` · `14.4.11.6` · `5.4.11.7`. Groups **4.7** (an additive `trackedObjects` array on R4) and **4.8** (R20/R21 tooling) are retired and not planned.
...
- **Three shared platform constraints** are the real contention, and they are scheduling constraints rather than dependencies: the **2-concurrent-Room deployment quota**, contended **four** ways — Phase 4's isolated ADA Room (groups 4.10–4.11) and its real-upstream Room (group 4.12), Phase 5's isolated IVI Room (group 5.9) and Phase 5's system test (group 5.10). **Phase 4's two are sequential** — group 4.12 takes the slot `5.4.11.7` releases — so the phase holds one at a time. Phase 3's `5.3.6.2` books none: it reads the ADA log Phase 4's Room already saved. Then the single registry namespace, which carries `m1-ada-bench:latest` alongside `m1-ada-ecu`, `m1-v2x-ecu`, `m1-netcheck` and `m1-r4-sim`; and the CI runner, where `phase4-ci.yml` gains three jobs.
...
| 5 | **Deployment quota — 2 concurrent Rooms** against **four** groups that each need one: Phase 4 groups 4.10–4.11 *(isolated ADA)* and group 4.12 *(real upstream)*, Phase 5 group 5.9 *(isolated IVI)*, Phase 5 group 5.10 *(system test)*. **Phase 4's two are sequential** — `13.4.12.1` takes the slot `5.4.11.7` releases. Serialize the rest and tear each down. Phase 3's `5.3.6.2` books none: it reads the ADA log `18.4.11.1` already saved | project-planner (scheduling) |
