...
| 10 | **The system test needs both nodes finished, so it is scheduled once and last.** [phase5_minh_tasks.md group 5.10](phase5_minh_tasks.md#task-group-510--system-verification-test) owns the 5-node blueprint; Phase 4 plans no full-chain deploy and its former group 4.6 is retired. The consequence to schedule around: group 5.10 cannot start until Phase 4's node-level evidence *and* Phase 5's app are both done, and it needs a Room slot that `5.4.11.7` releases | project-planner (scheduling) |
