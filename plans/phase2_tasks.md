...
- **Timestamp semantics are ruled by [m1-run-timing-and-event-triggering.md §6.2](../requirements/m1-run-timing-and-event-triggering.md), not by this brief's mapping line alone** — and that ruling is what fixes the swapped-timestamps defect recorded as **M1** in [phase2-4-pr3-review.md](doc/phase2-4-pr3-review.md). For `v2x_relayed`: `measured` and `received` both derive from the *same R2 message* (one clock domain), and `lastUpdated` is **always ADA's own `CLOCK_REALTIME` at store write** — never a foreign node's value. Cross-node timestamp arithmetic is forbidden; `rxTime` is a record value. **Reconcile before implementing:** §6.2 words `measured` as `rxTime + timeOfMeasurement`, while Phase 1 `9.1.4.3` already derives R2 `object.timeOfMeasurement` as an absolute `referenceTime + measurementDeltaTime`. Which of the two forms is correct is § Open items item 9 — the implementer flags it, and does not pick silently.
...
- **Which clock `now` reads is ruled by [m1-run-timing-and-event-triggering.md §6.2](../requirements/m1-run-timing-and-event-triggering.md)** — the HLD leaves `now - lastUpdated > TRACK_TIMEOUT_MS` unresolved and this closes it: **expiry is an interval, so it runs on `CLOCK_MONOTONIC`** (`steady_clock`), keeping a parallel monotonic stamp per track and comparing against that. `timestamps.lastUpdated` stays `CLOCK_REALTIME` — it is a log/wire value, not an interval operand. Reason to preserve: a host NTP step on the shared wall clock would otherwise expire every track at once mid-demo. `13.2.4.2` is unchanged — `now` is still a parameter; this fixes what the caller passes.
...
- Build stage: apt cmake/g++/git, configure and build the `ada_ecu` target only (tests excluded from the image build). Runtime stage: `/app/ada_ecu`, `/app/entrypoint.sh`, workdir `/app`, plus `tcpdump` + `coreutils` (base64) for the Phase 4 capture. `media/`'s COPY line lands in Phase 3 at `12.3.7.2`, and `models/`/`detector/`'s at `5.3.6.1` after it — one owner per line, media first so its layer stays cached; `tools/`, `tests/`, `doc/`, `schema/` never enter the runtime stage.
...
### [ ] `5.2.9.4` — Update `node-ada-ecu.md` with the D9 node config *(agent — writes in `requirements/car-sky-guide/`; **partially done — see status**)*

> **Status, 2026-08-03 — the deploy-blocking half is done; only the env table remains, and it blocks nothing.** [[project-architecture]] committed the three facts the deploy was waiting on (`62b69a2`): [node-ada-ecu.md § Blueprint node config](../requirements/car-sky-guide/node-ada-ecu.md#blueprint-node-config) now carries `"command": ["./entrypoint.sh"]` and `"capabilities": ["NET_RAW"]` — recorded there as *"always part of this node's config, never conditional"* — and § Build & push uses `registry.hackathon-2.carsky.io`. That is what [deploy-ada-ecu-walkthrough.md §8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) demanded **before deploying**, so this subtask is **no longer a prerequisite of [phase4_tasks.md](phase4_tasks.md) `5.4.10.5` or [phase3_tasks.md](phase3_tasks.md) `5.3.6.2`** — both were unblocked the same day ([phase4_tasks.md § Open items item 10](phase4_tasks.md#open-items--flags-no-phase-4-subtask-may-silently-close-them)).
>
> **What is left to do under this ID:** the §6 env-row additions below, plus a check that the two already-corrected fields still read as stated. Nothing waits on it — the env values a human types into the isolated Room come from `5.4.9.1`'s `blueprint-ada-isolated.json` and are diffed by `5.4.10.6`, not from this guide. Keep the ID, do not renumber it, and do not re-apply the three corrected facts as if they were still pending.
...
- ~~`"command": ["./entrypoint.sh"]` (was `["./ada_ecu"]`) and `"capabilities": ["NET_RAW"]`~~ — **done in `62b69a2`.** Both are required by the in-container capture (D9), and `NET_RAW` is unconditional on this node rather than contingent on a run wanting a capture. `entrypoint.sh` is product code in the image (the [V2X_ECU/entrypoint.sh](../V2X_ECU/entrypoint.sh) pattern), written by `5.2.7.1`; `capture.sh` by Phase 4 `6.4.4.1`.
...
- ~~Registry host corrected to `registry.hackathon-2.carsky.io` in § Build & push~~ — **done in `62b69a2`** (the `registry.carsky.io` lines were stale — Phase 0 O1, [phase1_tasks.md § Open items item 6](phase1_tasks.md#open-items--flags-no-phase-1-subtask-may-silently-close-them)).
