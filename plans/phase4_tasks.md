...
| **System test** — [phase5_minh_tasks.md group 5.10](phase5_minh_tasks.md#task-group-510--system-verification-test), which needs the finished IVI | the *"with bench scenarios live"* half of box 1 — relayed C originating in the real Scenario Player and decoded by the real V2X ECU, rendered by the real IVI |
...
|---|---|---|
| C tracked with `source = v2x_relayed` only, full R13 lifecycle | Phase 2 `2.2.3.1` + `13.2.4.3` · CI `15.4.5.1` · deployed `13.4.11.3` + `13.4.11.5` | node level here; *"bench scenarios live"* at [phase5 group 5.10](phase5_minh_tasks.md#task-group-510--system-verification-test) — § Open items item 4 |
