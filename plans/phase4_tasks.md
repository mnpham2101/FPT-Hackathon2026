...
> - **Design:** [ada-ecu-hld.md](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md) — §4 folder structure, §6 env tables, §12 test strategy; and in the [decision record](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md) **D4** (CRA interface, registry, database), **D5** (risk vocabulary, thresholds, edge-triggered emission, composition), **D7** (output stage), **D8** (evidence stream), **D9** (capture on this node), **D10** (clock domains), **D11** (the R22 band pair).
> - **Requirements:** [m1-cooperative-awareness.md §2](../documents/Requirements/m1-cooperative-awareness.md) R2, R4, R5, R6, R13, R14, R15, R18 and [m1-run-timing-and-event-triggering.md §7](../requirements/deprecated/m1-run-timing-and-event-triggering.md) R21, R22 — referenced by number, never restated.
...
> The business-logic half. `scene_composer` is geometry, `chained_collision` is rules; neither opens a socket, reads env, or formats a wire message ([HLD §3](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#mvc-separation)).
...
- Band table exactly [D5](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md#d5--risk-vocabulary-and-edge-triggered-emission) — **total and ordered, first matching row wins**, evaluated in this order:
...
- `RISK_TTC_WARN_S` is a designated configuration key with default `6` ([HLD §6](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#6-internal-components)); the plugin reads it from config like every other threshold.
...
> Controller layer ([D7](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md#d7--r15-output-stage) / [HLD §3](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#mvc-separation)): model → the view model the IVI consumes. `warning_builder` is **the only R4 producer in the node**, so the wire shape cannot drift from the schema the Phase 0 round-trip tests cover.
...
- The six checks, bounds exactly [HLD §12](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#12-test-strategy) — no bound invented here:
...
**Out of scope: K7.** It is read from the guest's logcat and the screen recording, not from any file this script takes ([HLD §12](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#12-test-strategy); [ivi-ecu-hld.md §12](../documents/Design/MODULE-DESIGN/IVI-ECU/ivi-ecu-hld.md#12-test-strategy)). Adding a logcat input here is a defect.
...
**Acceptance:** lane green on the pushed branch, both arms; the approaching arm observes ≥ 1 committed `risk_transition` and ≥ 1 `r4_tx`, and records in the job summary the observed band sequence **plus the composed `d_AC` and `ttc` series beside it**. The band sequence is recorded, not asserted — the risk defaults are unratified (§ Open items item 2), so a lane requiring a fixed sequence would red on every push. **Expect `low → medium`**: at the R22 pair — `RISK_NEAR_M` 60, `RISK_CRITICAL_M` 30 — `high` needs `d_AC ≤ 30 m` **or** `ttc ≤ 3 s` (D5 row 1, both clauses), which this profile's composed range does not reach ([D11](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md#d11--r22-run-choreography-the-run-origin-the-paced-clip-and-the-risk-band-pair)). Recording the series is what lets a reader tell a range trigger from a TTC trigger; a lane asserting `high` would red on correct behaviour.
...
>
> **The event names are the node's design, not an observed fact yet** — [§8.1 item 4](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these). `r2_ingest`, `own_sensor_ingest`, `track_transition`, `parse_reject`, `assessment`, `risk_transition` and `r4_tx` are literal strings from [HLD D8](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md#d8--r18-the-ada-half-of-the-evidence-stream); a check that finds none of them is more likely reading a node that emits different names than a node that did nothing. Compare against the log before concluding a failure.
...
| 10 | **The `geometry.vehicleC` null rule differs between the frozen contract and the ADA HLD.** [`contracts/r4-ada-ivi.schema.json`](../contracts/r4-ada-ivi.schema.json) states *"null until C is first tracked"*; [HLD §10.3](../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md#103-r4--the-message-set-to-the-ivi-ecu-produced) and D5 state *"null exactly when C's track has been erased"*. **This plan follows the contract** — contract-first is [CLAUDE.md](../CLAUDE.md) governing principle 1 — so `15.4.1.1`, `15.4.2.1`, `4.4.9.3` and `15.4.11.4` all treat a null `vehicleC` as legitimate both before C is first admitted and after its track is erased. The HLD's one-sided wording is reported, not followed | [[project-architecture]] (reconcile §10.3 and D5 with the contract) |
