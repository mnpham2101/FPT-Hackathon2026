...
**What this phase can run is two Rooms, and the second is what makes its boxes live.** The [isolated ADA Room](#task-group-410--isolated-ada-test-create-and-deploy-the-room-serves-r5-r6) mocks both of the ADA node's neighbours and retires every ADA-side unknown cheaply. The [alternative test](#task-group-412--alternative-test-real-bench-and-v2x-ecu-upstream-serves-r13-r15-r18) then keeps the ADA node byte-identical and replaces the **upstream** mock with the real thing — bench Scenario Player and V2X ECU — while the IVI sink mock stays downstream. Relayed C then originates in a real bench scenario and arrives as a real decoded CPM, which is exactly what the acceptance boxes mean by *"with bench scenarios live"*. **No IVI app is needed for that**; only the God view is. See § Where each Phase 4 box actually closes.
...
| `trackedObjects` | optional array of full R3 TrackedObjects | full B and C objects on the current ADA wire output |
...
The user subsequently ratified the additive optional `trackedObjects` field.
Current ADA output carries full B and C snapshots there while retaining C in
`object`; older consumers remain compatible because the field is optional.

**The additive-contract option is implemented and verified.** The original
reasoning below is retained as historical context for why geometry remains
...
The optional `trackedObjects` work is complete on the ADA producer; IVI
consumption/rendering remains Phase 5-owned.
...
> Task group **4.7** was ratified and completed as an additive optional
> `trackedObjects` extension. Task group **4.8** (R20/R21 run-alignment tooling)
> remains retired and not planned. IDs are not reused.
...
| # | Item | Owner |
|---|---|---|
| 1 | **Resolved for ADA:** the user ratified optional `trackedObjects`; schema, producer, tests and CarSky R4 evidence contain full tracked B and C. IVI consumption remains Phase 5-owned. | complete for ADA |
...
| 5 | **Phase 5 binding remains Phase 5-owned.** ADA now emits optional `trackedObjects`; IVI must tolerate presence and absence of that additive field and provide receive/parse evidence. | Phase 5 group 5.1 |
...
| 9 | **Room quota — two concurrent deployments across the whole account, contended four ways.** This phase's isolated Room (groups 4.10–4.11) and its real-upstream Room (group 4.12), Phase 5's isolated IVI Room (group 5.9) and Phase 5's system test (group 5.10). Phase 3's `5.3.6.2` books none — it reads this phase's log. **The two Phase 4 Rooms are sequential, not concurrent**: `13.4.12.1` takes the slot `5.4.11.7` releases, so this phase holds one slot at a time. Tear each down — `5.4.11.7` here, `17.5.9.16` and `19.5.10.8` in Phase 5 | [[project-planner]] (scheduling) |
...
*Phase 4 baseline planning contains 10 task groups. ADA implementation and
CarSky acceptance status are recorded in the implementation review and ADA
report; unchecked isolated-Room, IVI and pcap tasks remain future work. R20/R21
IDs remain retired and are not reused. Consolidated ADA+IVI view:
[ada-ivi-plan.md](ada-ivi-plan.md).*
