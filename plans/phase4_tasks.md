...
| 8 | **Closed 2026-08-04.** [§4.1](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#41-create-the-blueprint) no longer creates nodes over `/batch`, so the undesignated `batch-ada-isolated.json` payload it once named is gone. The Room is cloned from `baseline_phase1` and reduced on the canvas, and `blueprint-ada-isolated.json` is a config specification that is never imported or `POST`ed | — |
