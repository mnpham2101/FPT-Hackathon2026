...
**Three nodes run containers in the isolated Room. Two images serve them. Both images are built and pushed by CI — nothing is built by hand, by anyone, on any machine.** That sentence is [walkthrough §3](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#3-build-the-images-on-ci)'s opening claim, and it is binding on every subtask below: a developer changes code and pushes, and the lane rebuilds and republishes. There is no local `docker build` step anywhere in this phase, and a brief that implies one is wrong.
...
| `m1-ada-ecu:latest` | **ADA ECU**, `10.99.0.12` — the node under test | — | `ADA_ECU/` | `ada-ecu-image` | `phase2-ci.yml` | Phase 2 `5.2.8.1` (lane); contents by `5.2.7.1` + `6.4.4.1` + Phase 3 `12.3.7.2` / `5.3.6.1` |

**Two images, three nodes — the two mocks share one image and differ only by `ROLE`** ([§2.3](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#23-the-bench-image--one-image-two-roles), one image two roles). The bench image is therefore **deployed twice**, under two different `ROLE` values, from one push of one tag. That is deliberate packaging and it is also the single most misconfigurable thing in the Room: two nodes carrying the *identical* `image` string, told apart by one environment variable. `5.4.10.5` is where a human types it, and where that risk is called out.

**Between them, `ada-bench-image` and `ada-ecu-image` cover all three nodes' images.** Neither job alone is sufficient and neither substitutes for the other: `5.4.9.5` creates the first, `5.4.9.6` confirms the second against §3.2's build table, `5.4.10.1` confirms both went green, and `5.4.10.2` confirms **both tags** reached the registry. A green run with one tag missing leaves a node hanging in `Provisioning`.

**The ADA image's own deliverables are planned, not assumed.** `ADA_ECU/` today has no `Dockerfile`, no `entrypoint.sh` and no `capture.sh`. Phase 2 `5.2.7.1` writes the `Dockerfile`, `entrypoint.sh` and `.dockerignore`; this phase's `6.4.4.1` writes `capture.sh`; Phase 3 `12.3.7.2` and `5.3.6.1` add the `COPY` lines for the clip, the model and the detector. **`entrypoint.sh` is product code in the shipped image, not test scaffolding** — it is the `V2X_ECU/entrypoint.sh` pattern: launch `capture.sh` in the background, then `exec` the node binary so it becomes PID 1 and receives SIGTERM directly. That is why the node's `command` is `["./entrypoint.sh"]` rather than `["./ada_ecu"]`, in both blueprints.

**`capabilities: ["NET_RAW"]` is unconditional on the ADA node** — the user's ruling of 2026-08-03. It is not contingent on capture being wanted for a given run, not switched on only for evidence-gathering, and not a property of one blueprint: every ADA node config in this plan carries it, in the isolated Room and in the full chain alike. `capture.sh` degrading to packet counters when the capability is not honoured is a robustness property of the script, never a reason to omit the capability from a node config.
...
**Image-building jobs are exactly two, and together they build all three nodes' images**: `ada-bench-image` here (`m1-ada-bench:latest`, serving both mock nodes) and `ada-ecu-image` in `phase2-ci.yml` (`m1-ada-ecu:latest`). See § Every image the isolated Room runs is built and pushed by GitHub Actions. **No subtask in this plan builds or pushes an image from a developer machine.**
...
>
> **Where the bench lives, and why it is not a node folder.** `tools/ada-bench/` at the repository root, one image `m1-ada-bench:latest`, two roles selected by `ROLE` — the placement and its four rejected alternatives are [§2.4](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#24-where-the-bench-sources-live-and-why), argued there against [node-code-layout.md](../.claude/rules/node-code-layout.md). [tools/netcheck/](../tools/netcheck/) is the standing precedent: test equipment that deploys as a Container node, self-contained, its own Dockerfile, no imports from any node folder, no hardcoded peer addresses. **No subtask in this group writes inside `V2X_ECU/`, `IVI_ECU/` or `ADA_ECU/`** — the bench must be able to change without rebuilding the thing it tests. **The placement is sanctioned, not merely argued** — [[project-architecture]] amended [node-code-layout.md](../.claude/rules/node-code-layout.md) on 2026-08-03 to name `tools/` as the home for test equipment and ECU mocks, which retires the flag this group previously carried (§ Open items item 9).
>
> **Both of this group's image concerns are CI's, and neither is a hand build.** `5.4.9.5` adds the bench lane, `5.4.9.6` confirms the ADA lane — between them, all three nodes' images. The full mapping of image → node → job is § Every image the isolated Room runs is built and pushed by GitHub Actions, and nothing in this group repeats it.
>
...
- **Transcribe the ADA node's `command` and `capabilities` exactly as §2.2 states them, and treat neither as conditional.** `command: ["./entrypoint.sh"]` is right because `entrypoint.sh` is product code in the shipped ADA image — the `V2X_ECU/entrypoint.sh` pattern, written by Phase 2 `5.2.7.1`, which backgrounds `capture.sh` and `exec`s the binary so it is PID 1. `capabilities: ["NET_RAW"]` is **unconditional on this node** (user ruling, 2026-08-03) — write it whether or not any given run wants a capture. [node-ada-ecu.md](../requirements/car-sky-guide/node-ada-ecu.md) was corrected to both values, plus the current registry host, by [[project-architecture]] on 2026-08-03, so the guide and this file now agree and [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) is closed — see § Open items item 10.
...
**What this job covers, and what it does not.** This lane builds **one image, `m1-ada-bench:latest`, which two of the Room's three nodes run** — the V2X Bench mock under `ROLE=v2x_mock` and the IVI Sink mock under `ROLE=ivi_mock` ([§2.3](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#23-the-bench-image--one-image-two-roles), one image two roles). **One build, one push, one tag, deployed twice** — the lane does not build a second mock image and must not be extended to, because the roles are an env-var selection inside `entrypoint.sh` (`5.4.9.4`), not separate artifacts. The third node's image, `m1-ada-ecu:latest`, is `ada-ecu-image`'s and is confirmed by `5.4.9.6`; **between the two jobs, every image the isolated Room runs is built and pushed by GitHub Actions and none is built by hand** (§ Every image the isolated Room runs is built and pushed by GitHub Actions).
...
**What this job covers.** `m1-ada-ecu:latest` — the **third** of the Room's three node images, and the only one this lane builds. With `5.4.9.5`'s `ada-bench-image` it completes the set: **two CI jobs, two images, three nodes, zero hand builds.** If this lane is wrong or missing, the node under test cannot be deployed at all, and the two mocks alone prove nothing.
...
**Three nodes, three image strings to type — and two of them are identical.** This is §4.3's three-row image table, reproduced here because it is the one thing this subtask exists to get right; every other value is cited, not restated.
...
**The risk this brief exists to call out: rows 1 and 3 carry the *same* image string and differ only by one environment variable.** Two nodes, one tag, two roles — pulled from a single CI push of `m1-ada-bench:latest` (§ Every image the isolated Room runs is built and pushed by GitHub Actions). Nothing on the canvas distinguishes them, so the misconfiguration is invisible on inspection and is the most likely single defect in this step. Three ways it goes wrong, all of them looking like something else:

- **Both mocks given the same `ROLE`.** Two emitters and no sink (or two sinks and no emitter). With `ROLE=v2x_mock` on both, the ADA node ingests fine and `15.4.11.4`'s `sink.log` never exists; with `ROLE=ivi_mock` on both, `2.4.11.2` finds `r2_ingest` at 0 and reads like a routing fault.
- **`ROLE` misspelled on either.** §4.3's fourth row: the container exits at start and the **restart count climbs** — the [§6](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#6-troubleshooting) symptom for it. `v2x_mock` and `ivi_mock` are exact strings, underscore not hyphen.
...
**Scope:** the table above plus §4.3's four-row *"values that decide whether anything works at all"* table — `TARGET_HOST`, `TARGET_PORT` = `V2X_LISTEN_PORT`, `IVI_ECU_HOST`/`IVI_ECU_PORT` = the sink's `LISTEN_PORT`, and `ROLE`. Each fails in a way that looks like something else, and the symptoms are that table's, not this brief's. **This step stays a hand edit even though `5.4.10.3` created the nodes with config attached** — §4.3: the API has no update route, so every correction after creation is an Inspector edit. Click the node, edit, click empty canvas to commit.

**The ADA node's `command` and `capabilities`, and why neither is a judgement call:**

- `command: ["./entrypoint.sh"]` — relative to the image workdir `/app`; `/entrypoint.sh` does not exist and the container dies at start (§4.3's closing note). `entrypoint.sh` is **product code in the ADA image**, mirroring [V2X_ECU/entrypoint.sh](../V2X_ECU/entrypoint.sh): it backgrounds `capture.sh` and `exec`s the binary so the node is PID 1 and takes SIGTERM directly. It is written by Phase 2 `5.2.7.1`, not by this subtask.
- `capabilities: ["NET_RAW"]` — **unconditional on this node** (user ruling, 2026-08-03; [node-ada-ecu.md § Blueprint node config](../requirements/car-sky-guide/node-ada-ecu.md#blueprint-node-config): *"always part of this node's config, never conditional"*). Type it whether or not this particular run wants a capture. Omitting it does not fail the deploy — it silently degrades `capture.sh` to a packet counter, which is the worst kind of miss.

Both values now agree across [node-ada-ecu.md](../requirements/car-sky-guide/node-ada-ecu.md#blueprint-node-config), [walkthrough §2.2](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#22-the-blueprint-definition-and-where-it-lives) and `5.4.9.1`'s blueprint file, so **the reconciliation [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) demanded before deploying is closed** ([[project-architecture]], commit `62b69a2`) — this subtask is no longer blocked on it. § Open items item 10.

**Acceptance:** every field typed, then proven by `5.4.10.6`'s read-back rather than by the Inspector's truncated fields (§4.4 is explicit that the read-back is what counts). Record in the run doc which values were typed, **and the `image` + `ROLE` pair for each of the three container nodes explicitly** — that pairing is what a later reader needs to tell the two bench nodes apart.
...
**Three image/`ROLE` assertions this diff must make explicitly**, because they are what `5.4.10.5` most likely got wrong and what a field-by-field diff would otherwise pass over: `.11` and `.13` both carry `image` = `…/m1-ada-bench:latest`; their `ROLE` values are `v2x_mock` and `ivi_mock` respectively and **are not equal to each other**; and `.12` carries `image` = `…/m1-ada-ecu:latest` with `command: ["./entrypoint.sh"]` and `capabilities: ["NET_RAW"]` present. Report any of the three as a mismatch by name.
...
Lane E (isolated Room - the bring-up lane; touches no file the fusion code touches)
  ...
  4.10 5.4.10.1 (USER) ──► 5.4.10.2 ──► 5.4.10.3 ──► 6.4.10.4 (USER) ──► 5.4.10.5 (USER)
...
| 9 | ~~**`tools/ada-bench/` is implementation code outside the four node folders.**~~ **CLOSED 2026-08-03** — [[project-architecture]] amended [node-code-layout.md](../.claude/rules/node-code-layout.md) with a [§ `tools/` — test equipment and ECU mocks](../.claude/rules/node-code-layout.md#tools--test-equipment-and-ecu-mocks) section (commit `99ae675`) that names `tools/ada-bench/` in its table, alongside `tools/netcheck/` and `tools/comms_check/`, and applies the same build rules the four node folders carry. The placement group 4.9 uses is sanctioned by the rule, not merely argued from [walkthrough §2.4](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#24-where-the-bench-sources-live-and-why). No further action | **closed** — [[project-architecture]] `99ae675` |
| 10 | ~~**[node-ada-ecu.md](../requirements/car-sky-guide/node-ada-ecu.md) states three facts the walkthrough contradicts, and the deploy is blocked on the reconciliation.**~~ **CLOSED 2026-08-03** — [[project-architecture]] corrected the guide (commit `62b69a2`): [§ Blueprint node config](../requirements/car-sky-guide/node-ada-ecu.md#blueprint-node-config) now carries `command: ["./entrypoint.sh"]` and `capabilities: ["NET_RAW"]` (stated there as *"always part of this node's config, never conditional"*), and § Build & push uses `registry.hackathon-2.carsky.io`. The guide, [walkthrough §2.2](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#22-the-blueprint-definition-and-where-it-lives)/[§9](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#9-quick-reference) and `5.4.9.1`'s blueprint file now agree, so [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) is satisfied and **`5.4.10.5` and Phase 3 `5.3.6.2` are no longer blocked**. **Phase 2 `5.2.9.4` keeps only its unblocking half** — the §6 env-row additions to the same guide — which gates nothing: the env values a human types come from `5.4.9.1`'s blueprint file, diffed by `5.4.10.6` | **deploy-blocking half closed** — [[project-architecture]] `62b69a2` · residual env-table half: Phase 2 `5.2.9.4`, blocking nothing |
...
| 13 | **[§8.1](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) items that authoring cannot retire, and where each first bites.** Item 1 (the whole route is unexercised) — the lane as a whole. Item 2 (none of the [§1.3](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#13-deliverable-prerequisites) ADA deliverables written) — group 4.10's entry condition; they are Phases 2–4 work planned above. Item 4 (the `[EVT]` names are design, not observation) — `2.4.11.2`, `13.4.11.3`. Item 6 (arm64 wheels for the detector) — Phase 3 `12.3.1.1`, and `5.4.9.6`'s 360-minute timeout check. Item 9 (a `/batch`-created 4-node blueprint accepting hand-drawn pins and then validating) — `6.4.10.4`, whose `validate` pass is the first proof of it. Item 10 (the defaults may produce no risk transition) — `13.4.11.3`, contingency `14.4.11.6`. Item 11 (detector frame rate on the Room's CPU) — Phase 3 `5.3.6.2`. Item 12 (whether the platform actually **honours** a requested `NET_RAW` — the plan always requests it, on the ADA node unconditionally) — `15.4.11.4`'s `[CAP]` criterion. **None is filled in by guessing; each is reported from the subtask that hits it** | per subtask |
...
*Updated 2026-08-03 by project-planner on three user rulings. **(1)** Added § Every image the isolated Room runs is built and pushed by GitHub Actions — two CI jobs, two images, three nodes, nothing built by hand — and carried it into the § CI ruling, group 4.9's preamble, `5.4.9.5`, `5.4.9.6`, `5.4.10.1` and `5.4.10.2` (both tags, not one). **(2)** `capabilities: ["NET_RAW"]` is unconditional on the ADA node wherever it appears — `5.4.9.1`, `5.4.6.2`, `6.4.4.1`, `5.4.10.5` and open item 13. **(3)** Open items 9 and 10 closed against [[project-architecture]]'s commits `99ae675` (`tools/` sanctioned in `node-code-layout.md`) and `62b69a2` (`node-ada-ecu.md` corrected), which unblocks `5.4.10.5` and Phase 3 `5.3.6.2` from Phase 2 `5.2.9.4`. `5.4.10.5` rewritten around the three node→image pairings and the same-image-different-`ROLE` risk; `5.4.10.6` gained the matching read-back assertions. No ID added, removed or renumbered.*
