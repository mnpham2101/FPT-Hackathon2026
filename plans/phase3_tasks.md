...
> **Route named 2026-08-03.** This subtask predates [deploy-ada-ecu-walkthrough.md](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md) and left the Room unspecified. **It runs on the isolated blueprint of [§2.1](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#21-topology)** — bridge + V2X bench mock + ADA + IVI sink mock — which is the cheapest Room that gets this node `Running`, and which [phase4_tasks.md group 4.10](phase4_tasks.md) stands up. The detector needs no neighbour at all for this measurement, so if that Room is already up for group 4.11, **take the reading from it rather than spending a second slot** ([§8.1 item 13](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these): two deployments across the whole account). The full 5-node blueprint is not needed here and never was.
>
> **The AI half is [[car-sky]]'s, per [§7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#7-work-division-between-ai-and-human).** Polling node phases to `Running` and saving the node log with the mandatory `?container=user` are AI rows — `5.4.10.8` and `18.4.11.1` carry those briefs. The node-config edit and the deploy are Human rows no agent performs; this *USER-MANUAL* subtask keeps them plus the measurement and its record.
>
> **This is where [§8.1 item 11](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) bites** — *the detector's frame rate on the Room's CPU allocation. If it falls far below the configured stride, ego's own track may expire between updates and the composed geometry disappears.* A rate below KPI 3 is therefore not only a Phase 3 miss; it is a Phase 4 failure mode for `13.4.11.3`'s own-sensor half. Record the measured rate whichever way it lands.

**Scope:**

- Node config per [node-ada-ecu.md § Blueprint node config](../requirements/car-sky-guide/node-ada-ecu.md#blueprint-node-config): image `registry.hackathon-2.carsky.io/m1-ada-ecu:latest`, `command: ["./entrypoint.sh"]`, `capabilities: ["NET_RAW"]`, the §6 env set. **`capabilities: ["NET_RAW"]` is unconditional on this node** — the guide's own words are *"always part of this node's config, never conditional"* — so it is typed for this measurement run too, even though nothing here reads a capture. **The `5.2.9.4` prerequisite this subtask previously carried is discharged**: [[project-architecture]] corrected the guide's `command`, `capabilities` and registry host on 2026-08-03 (commit `62b69a2`), which is what [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) required **before deploying** ([phase4_tasks.md § Open items item 10](phase4_tasks.md#open-items--flags-no-phase-4-subtask-may-silently-close-them)). New Deployment → Deployment Viewer shows the ADA node **Running**, restart 0; mind the 2-deployment quota. A node stuck in `Provisioning` means the image could not be pulled — [§4.5](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#45-deploy), then [carsky-room-diagnostics](../.claude/skills/carsky-room-diagnostics/SKILL.md), never a blind redeploy.
- **The image comes from CI, never from a hand build** — `ada-ecu-image` in `phase2-ci.yml` builds and pushes `m1-ada-ecu:latest` on every push. It is one of the two jobs that between them build all three node images the isolated Room runs; the other, `ada-bench-image`, builds `m1-ada-bench:latest` for the two mock nodes ([phase4_tasks.md § Every image the isolated Room runs is built and pushed by GitHub Actions](phase4_tasks.md#every-image-the-isolated-room-runs-is-built-and-pushed-by-github-actions)). If this reading is taken off group 4.10's Room, both images are already live there.
...
>
> **The binding content row is not negotiable and no editing step can create it:** vehicle **B** visible and occluding the ego lane at roughly **10–40 m** in **≥ 90%** of frames.
>
> **C is not a content row at all — clarified 2026-08-02.** C is *synthetic*: it exists only as a position the bench asserts over V2X, never as a filmed object, so "C never visible" is satisfied by construction rather than by finding footage. What the clip must avoid is a **decoy** — a vehicle held in the ego lane beyond B, at the range the bench claims C occupies, long enough to be admitted as an `own_sensor` track. Adjacent-lane and oncoming traffic are not decoys and are welcome; a brief or partial far-ahead sighting is not one either, since it survives neither the R13 admission state machine nor a position match against C. Full reasoning and the obligations it puts on the scenario config and `check_zero_c.py`: [ego-b-occluding-c.source.md § C is synthetic](../ADA_ECU/media/ego-b-occluding-c.source.md#c-is-synthetic--what-the-criterion-actually-constrains).
...
3. **No decoy in the ego lane**: **B is the frontmost visible vehicle in the ego lane for the entire segment** — nothing held ahead of B in that lane, and nothing visible past or through B. Vehicles in adjacent lanes and oncoming traffic are *not* C and are acceptable. A vehicle **sustained** ahead of B in the ego lane disqualifies the segment (re-cut around it, or reject the clip); a single-frame or partial far-ahead sighting does not, because it cannot be admitted as a track. C itself is synthetic and is never in the footage — see the group preamble.
...
*Amended 2026-08-03 by project-planner against [deploy-ada-ecu-walkthrough.md](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md), stage 2 of [walkthrough-driven-delivery.md](../.claude/rules/walkthrough-driven-delivery.md). **One subtask corrected, none added and none renumbered:** `5.3.6.2` left its Room unspecified and folded two AI rows into a user step — it now names the isolated blueprint (§2.1) as its Room, hands the phase-polling and log-saving rows to [[car-sky]] per §7, makes Phase 2 `5.2.9.4` a hard prerequisite per §8.1 item 7, and records that §8.1 item 11 (detector frame rate on the Room's CPU) is the unknown it retires. The deploy itself and the node-config edit stay human work no agent performs.*

*Amended 2026-08-03 (later the same day) by project-planner on two user rulings. `5.3.6.2`'s `5.2.9.4` prerequisite is **discharged** — [[project-architecture]] corrected [node-ada-ecu.md](../requirements/car-sky-guide/node-ada-ecu.md) in commit `62b69a2`, which is what §8.1 item 7 required before deploying — so this subtask is no longer blocked by Phase 2 work. Its brief also now states that `capabilities: ["NET_RAW"]` is **unconditional** on the ADA node, and that the node image comes from the `ada-ecu-image` CI job rather than any hand build. No ID added, removed or renumbered.*
