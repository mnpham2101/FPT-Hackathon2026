...
- Node config per [node-ada-ecu.md § Blueprint node config](../requirements/car-sky-guide/node-ada-ecu.md) as updated by `5.2.9.4`: image `registry.hackathon-2.carsky.io/m1-ada-ecu:latest`, `command: ["./entrypoint.sh"]`, `capabilities: ["NET_RAW"]`, the §6 env set. **`5.2.9.4` is a hard prerequisite, not a nicety** — that guide today still says `command: ["./ada_ecu"]`, carries no `capabilities` and names the stale `registry.carsky.io` host, and [§8.1 item 7](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#81-confirm-before-relying-on-these) requires the discrepancy resolved with its owner **before deploying** ([phase4_tasks.md § Open items item 10](phase4_tasks.md#open-items--flags-no-phase-4-subtask-may-silently-close-them)). New Deployment → Deployment Viewer shows the ADA node **Running**, restart 0; mind the 2-deployment quota. A node stuck in `Provisioning` means the image could not be pulled — [§4.5](../requirements/car-sky-guide/deploy-ada-ecu-walkthrough.md#45-deploy), then [carsky-room-diagnostics](../.claude/skills/carsky-room-diagnostics/SKILL.md), never a blind redeploy.
