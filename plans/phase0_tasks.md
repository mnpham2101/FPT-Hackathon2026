...
- Job `netcheck-image`: builds `m1-netcheck:latest` and pushes it to `registry.hackathon-2.carsky.io` when `CARSKY_ZOT_API_KEY` is set, per [CLAUDE.md § Repository layout](../CLAUDE.md). Storing that secret is the Human step `5.0.8.5`; the missing credential is flagged to the user, not worked around.
