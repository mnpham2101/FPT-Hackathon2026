...
- [ ] Golden-vector CPMs decode correctly; the malformed-input corpus is fully rejected with zero crashes (R9). — **partly:** golden decode and zero crashes are **CI-proven** (Phase 0 `1.0.2.5` + `9.1.4.4`; `9.1.4.5` drives the corpus through the **real** Vanetza codec, green in run 30697863324). *Malformed* is settled by the architecture ruling of 2026-08-01 (§ Open items item 8) to mean **structurally invalid** input, and the amended `9.1.4.5` corpus is categorised accordingly: 6 structurally-malformed cases dispositioned `Reject` (`empty`, `truncated-nominal`, `truncated-mid-object`, `bit-flipped-payload`, `random`, `oversized`) and 4 valid-but-different inputs dispositioned `ToleratedControl` — the 3 header edits, which profile §3 freezes as ignored on decode ([contracts/r1-cpm-profile.md](../contracts/r1-cpm-profile.md) §3), plus `trailing-garbage`. Those 4 are **correctly-tolerated negative controls, not failures**. Still `[ ]` for one reason only: the 3 new fixtures' dispositions are **predicted from the wire-layout analysis, not measured** (no C++ toolchain on the authoring host), so the malformed half closes when the next `v2x-core-build` run confirms every `Reject`-dispositioned case rejects — § Remaining work.
...
**Phase 1 acceptance state 2026-08-01: 3 of the 9 boxes closed (R7, R8, D7), 5 partly closed (R5, R6, R9, R11, R2), 1 open (Demo).** Every unclosed clause now needs either a live Room or the one outstanding R9 CI confirmation — § Remaining work.

### Remaining work

Every subtask not listed below is implemented and closed. Two primary verifications: **CI run `30697863324` on commit `16b8674`, all 8 lanes green** over the whole tree at that commit (Waves A–D), and **CI run `30698630956` on commit `7a02fb5`, all 10 lanes green** — adding the two node-image lanes, so both images demonstrably build for `linux/arm64`, and re-confirming the eight earlier lanes on a later tree. The full 10-lane run completes green. Only two commits are covered by no CI run: `3202c72` (the `9.1.4.5` amendment adding three malformed fixtures whose dispositions are predicted, not measured — `7a02fb5` is its parent) and this docs-only closeout — which is exactly the R9-confirmation gap below. Nothing else in this plan is unverified; what remains is 5 subtasks plus that one CI confirmation, none of it new application code and none of it buildable off-platform.
...
| `9.1.4.5` (amended) | *agent* (CI) | Confirm the amended R9 corpus: the next `v2x-core-build` run must show every `Reject`-dispositioned case rejecting. The 3 new fixtures' dispositions are predicted, not measured — a wrong prediction turns the lane RED and is fixed by relabelling that one case's `Disposition`. Closes the R9 box's malformed half and § Open items item 8's residual. |
...
> Split per Phase 0 group 0.8: [[car-sky]] for registry work, USER-MANUAL for Nydus UI (blueprint `command`/`capabilities`/env edits are manual — V2X HLD §11 item 4; REST cannot edit node config). **[[car-sky]] is not spawnable this session — 5.1.10.1 is planned and tracked, execution deferred; nothing in groups 1.1–1.9 blocks on this group.** Evidence accumulates in `plans/doc/phase1-comms-run.md` (created by 5.1.10.1). Standing requirement: images single-platform `linux/arm64` ([phase0-smoke-test-run.md](doc/phase0-smoke-test-run.md)).

### [ ] `5.1.10.1` — Push the three images to the CarSky registry *(car-sky — deferred)*
...
| 4 | [[car-sky]] not spawnable in any session so far — group 1.10 execution deferred; also requires `CARSKY_ZOT_API_KEY` as a repo secret for the CI push path. Still open, and now the whole of the phase's remaining scope (§ Remaining work). Note since run `30698630956`: both image lanes build green with the push step already wired and gated on that secret, so setting the secret alone would let the lanes push by themselves — a real path to closing `5.1.10.1` without the [[car-sky]] agent (untested, since the secret has never been present) | orchestrator / user |
