...
Every subtask not listed below is implemented and closed. Primary verification: **CI run `30697863324` on commit `16b8674`, all 8 lanes green** over the whole tree at that commit (Waves A–D). Three later commits are covered by no CI run — `e02fb7c` (F1 Dockerfile amendment), `df90774` (the two image lanes) and the 2026-08-01 `9.1.4.5` amendment (three added malformed fixtures whose dispositions are predicted, not measured) — which is exactly the image-build and R9-confirmation gap below. Nothing else in this plan is unverified; what remains is 8 subtasks plus that one CI confirmation, none of it new application code.
