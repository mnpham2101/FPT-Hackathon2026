...
**Follow-up — this loop is R20's bench half** ([m1-run-timing-and-event-triggering.md §2(d)](../requirements/m1-run-timing-and-event-triggering.md)): scenario time is a tick counter (`t = cycle_tick * period`) advanced by a fixed `sleep(period)` with no drift correction, so it diverges from wall time by the per-tick work cost, ~1 % accumulating and unbounded over a run; and the `[TX]` line carries **no timestamp at all**, which is why K5 of §6.4 cannot be measured today. **Neither half is planned in any phase** — not the `mono_ms` field and not deadline scheduling against `CLOCK_MONOTONIC` (§ Open items item 10). R20/R21 are unratified and gate no acceptance box; until the user accepts them, a red K5 is unmeasurable rather than measured, and this loop's drift is a known, visible, unscheduled property.
...
**Scope:** push path = the CI lanes (5.1.8.2 + existing `netcheck-image`) once `CARSKY_ZOT_API_KEY` is a repo secret, or direct build/push; verify tags present via the registry API; create `plans/doc/phase1-comms-run.md` recording pushed digests. Registry host is `registry.hackathon-2.carsky.io` ([zot-registry-api-key.md § Registry host caveat](../requirements/car-sky-guide/zot-registry-api-key.md#registry-host-caveat-open-item-o1)).
...
| 6 | ~~`node-v2x-ecu.md` / `node-scenario-player.md` "Build & push" sections still cite the dead registry host — guide touch-up owed by architecture~~ — **CLOSED 2026-08-03.** [[project-architecture]] corrected every live operational reference to `registry.hackathon-2.carsky.io` across nine commits: both node guides (`c9ac50e`, `e5d5507`), [carsky-4-node-blueprint.md](../requirements/car-sky-guide/carsky-4-node-blueprint.md) (`ec28903`), both M1 blueprint JSONs (`faae9ee`, `c7f1b7c`), the [[car-sky]] agent spec (`3bd8524`), the deploy-preflight skill (`d9ea9dd`), the ADA HLD (`c7af3ce`), and [zot-registry-api-key.md](../requirements/car-sky-guide/zot-registry-api-key.md#registry-host-caveat-open-item-o1) (`53e2b2a`), whose caveat is now written as resolved fact. The two agent-facing files mattered most — they were instructing [[car-sky]] to log in to the dead host. Phase 0 **O1** (which host serves Zot) was itself answered 2026-07-31 by `5.0.8.2`; this item carried only the documentation debt, and it is discharged. **The resolution is environment-scoped**: the registry host mirrors the round's environment host, as the REST base URL does, so a later round re-derives it rather than assuming `hackathon-2` | **closed** — [[project-architecture]], nine commits |
...
| 10 | **R20 is not planned in any phase — neither half.** `11.1.6.7`'s fixed-sleep loop drifts ~1 % accumulating ([§3.4](../requirements/m1-run-timing-and-event-triggering.md)), the `[TX]` line carries no `mono_ms`, and the bench `start_delay_s` of the §4.2 B-1 readiness pick is unscheduled with them. §8(2) rates the deadline fix plus the `mono_ms` field at ~1 h and recommends doing it regardless, but §8(1) puts R20/R21 **behind** every Phase 3 and Phase 4 acceptance box and **the user has not accepted them**, so nothing is scheduled. If accepted they become new work with new IDs. **Visible and unscheduled, not absorbed** | **user** (accept/reject R20/R21 per §8(1)) |
