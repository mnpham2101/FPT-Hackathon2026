...
1. [documents/Requirements/m1-cooperative-awareness.md](documents/Requirements/m1-cooperative-awareness.md) — **the authoritative document, in whole**: §1 project description (ultimate authority on goals/scope), §2 enumerated requirements R1–R19 (each with definition, acceptance, tech stack), §3 technical solutions, §4 standing user decisions.
...
|---|---|---|
| [project-researcher](.claude/agents/project-researcher.md) | Requirements, feasibility, tech-stack selection, [documents/KnowledgeBase/](documents/KnowledgeBase/) | Architecture, task breakdown, code |
...
- **Code: one top-level folder per R5 node** — [Scenario_Player/](Scenario_Player/) (bench, R11, Python, image `m1-scenario-player:latest`) · [V2X_ECU/](V2X_ECU/) (R1, R7–R9, C++17, `m1-v2x-ecu:latest`; R10 deferred to a future milestone) · [ADA_ECU/](ADA_ECU/) (R3, R12–R15, C++17 core + Python detector subprocess, `m1-ada-ecu:latest`) · [IVI_ECU/](IVI_ECU/) (R4, R16–R17, Kotlin/Jetpack Compose, APK via Gradle). No product code lives outside them; the bench is a node of its own, so R11 code never lands in `V2X_ECU/`. Per-node facts — image tag, entrypoint, env set, blueprint config: [requirements/car-sky-guide/](requirements/car-sky-guide/); deploy and test procedure: [documents/Delivery/](documents/Delivery/).
- **Node build rules.** Each node folder is **self-contained and independently buildable** — own `Dockerfile` at its root, own dependency manifest, own tests — and builds from the repo root, e.g. `docker build -t m1-scenario-player:latest Scenario_Player/`. **No cross-node source imports**: folders couple only through the frozen R1–R4 contracts, so a shared artifact two nodes need is a contract deliverable, never a relative import. **No hardcoded tunables** (governing principle 5): peer addresses, ports, thresholds and cadences come from env or config, injected by the blueprint node config. Unit tests live inside the node folder they test. A CI image lane pushes only when `CARSKY_ZOT_API_KEY` is set — **a green lane is not evidence the image reached the registry**; read the run's push notice or query the registry.
...
- [requirements/](requirements/) — researcher reports ([format](.claude/rules/research-report-format.md)); `m1-cooperative-awareness.md` is the live one; [future/m1-future-features-register.md](documents/Requirements/future/m1-future-features-register.md) mirrors its § Future developments ([Vietnamese translation](documents/Requirements/future/m1-future-features-register.vi.md) — non-authoritative; the English register wins on conflict, and changes land there first).
...
- **Human-facing publications, one top-level folder each, self-contained with its own content, design system and generator** — [presentation/](presentation/) holds the decks, their `template/` and `slide-build-tool/` ([deck-authoring-conventions.md](.claude/rules/deck-authoring-conventions.md)); [website/](website/) holds the static hub site, its `css/`, `js/` and `build-pages.py` ([website/README.md](website/README.md)). Both render [documents/](documents/) and neither owns it. They are not [tools/](tools/).
- [tools/](tools/) — **test equipment and ECU mocks**, one folder per tool: diagnostic tools, simulators, and the containers that stand in for another ECU so a node can be exercised alone in a reduced blueprint. A container that *replaces* a node belongs here; a node's own real image never does, and a mock of node X does not live in X's folder. These are not R5 nodes — no requirement number, no place in the full blueprint — but they follow the node build rules above, and take their contract fields from the node's `contracts/` file rather than a private copy.
- [.claude/rules/](.claude/rules/) — standing process rules (task planning, report format, requirement quality, solution selection, HLD format, CI lane placement, PR review format, deck conventions, reasoning visibility).
- [.claude/skills/](.claude/skills/) — reusable procedures (requirement analysis, HLD, environment research, CarSky deploy/diagnostics, markdown style).
- [.claude/agents/](.claude/agents/) — the five agent specs. [.claude/prompts/](.claude/prompts/) — saved prompts. [.claude/references/](.claude/references/) — cached external evidence.
- [documents/](documents/) — **every document about a node, filed by what it is rather than by where its code sits.** The test is what the document is *about*, not what it is called or which node consumes it: a study of what the platform serves as camera input is knowledge even though one node reads it; a port plan a node's HLD names as its own deliverable is design even though it reads like research. A folder appears when its first document lands, so a missing one means "nothing written yet". The three destinations and their owners:
