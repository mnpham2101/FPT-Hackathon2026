...
| **F3** | **Python bench → C++ Vanetza encoder path is unresolved** — standing open item in [node-code-layout.md](../../../.claude/rules/node-code-layout.md#scenario_player-specifics-r11). | Ranked for [[project-architecture]] to decide in the R11 HLD: (1) a small C++ `cpm_encode` helper built from the same Vanetza target, invoked by Python over stdin/stdout JSON — reuses the exact R1 codec, mirrors the R12 subprocess pattern already sanctioned; (2) pybind11 binding — more toolchain; (3) pre-encoded vectors + byte patching — drifts, fails R11's "different configs → different streams". |
