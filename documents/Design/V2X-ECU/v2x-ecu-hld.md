...
| [baseline-connectivity-smoke-test.md](../../Delivery/baseline-connectivity-smoke-test.md) | The capture technique D5 adopts — in-container tcpdump, `NET_RAW` flat in `capabilities`, the `/proc/net/dev` counter fallback — and the View-Log-as-retrieval model that both D5's pcap export and D7's on-platform check rest on; `tools/netcheck/` as the R2 observation sink (D6) |
