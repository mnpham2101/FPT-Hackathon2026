...
## D8 — A repeating waiting/two-vehicle/three-vehicle demo cycle, period-locked to the ADA clip

A second, opt-in demo choreography alongside D7's continuous-approach cycle: `scenarios/r19-demo-cycle.yaml` carries a `phases` key (`waiting_s`, `two_vehicle_s`, `three_vehicle_s`) instead of `duration_s`/`object`, selecting `player/phased_generator.PhasedGenerator` in place of `player/generator.Generator` (`main`'s composition choice, on `ScenarioConfig.phases is not None`). Each cycle repeats: `waiting_s` of silence — no CPM sent, the IVI's waiting state — then `two_vehicle_s` of CPM sampled from `two_vehicle_object` (C held static beyond the R13 30 m admission gate, so only A and B are perceptually present), then `three_vehicle_s` of CPM sampled from `three_vehicle_object` (C already inside the gate and closing, so the R19 warning holds for the whole phase).

- **The kinematic model is untouched (D3).** `player/scenario.py` still models one constant-velocity object in B's frame; `phased_generator` builds two ordinary `Scenario` instances (one per active phase, via `dataclasses.replace` swapping only `object`) and samples each from phase-local time 0. Phase selection and the silence period are controller-layer (D8's addition to the UI-logic layer, §3 MVC table), never a branch inside the model.
- **The cycle length must be an integer multiple of the ADA video clip's fixed 10.0 s length**, for the same reason D7 binds `duration_s` to the clip exactly: the ADA detector's own-sensor video loop (`ego-b-occluding-c.mp4`, `ada-ecu-design-decisions.md` D11) runs independently of the CPM stream and re-aligns with the bench only at a shared period boundary. `r19-demo-cycle.yaml` sums to 20.0 s (10 + 5 + 5), two clip loops exactly — any total that is not a multiple of 10.0 s lets the clip wrap mid-`three_vehicle` phase, dropping the assessment to `low` on ADA's `b_unknown` path (D7/D11), which is why the requested 10 + 4 + 5 = 19 s shape was padded to 20 s rather than implemented as asked.
- **Silence, not an empty CPM, represents "no C."** The frozen R1 profile carries exactly one mandatory `PerceivedObject` per message (SP HLD §10) — there is no wire shape for "B present, C absent." `two_vehicle_s` therefore still sends a real CPM (B is present) but keeps C beyond the R13 gate so ADA's own admission state machine — not a missing field — is what withholds the C track, the same mechanism `c-out-of-range.yaml` already establishes for D3.
- **A new file, not an edit to `default.yaml`.** D7's continuous-approach cycle and its pinned acceptance values (`test_config.py::TestCommittedScenarioVariants`, `test_streams_differ.py`) stay exactly as they are; the phased cycle is a second, independently selectable scenario (D3 — variants are files).

| Rejected alternative | Why |
|---|---|
| The exact 10 + 4 + 5 = 19 s cycle as first requested | Coprime with the clip's 10.0 s loop — most `three_vehicle` repetitions would straddle a clip wrap and intermittently drop the warning, the opposite of the intended demo |
| An empty/omitted `object` field to represent "no C" | The frozen R1 profile requires exactly one `PerceivedObject`; the wire has no "absent" shape, and changing that is a contract re-freeze, not a scenario change |
| Branching inside `player/generator.py` on an optional `phases` field | Collapses two independently-testable loop shapes into one function and risks regressing the existing continuous-cycle tests; a separate `phased_generator` module keeps both paths independently verifiable |
