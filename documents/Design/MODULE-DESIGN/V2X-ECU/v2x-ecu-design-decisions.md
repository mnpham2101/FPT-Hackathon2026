...
This node stamps the only timestamp another node records, so its clock discipline is a contract obligation rather than an implementation detail ([m1-run-timing-and-event-triggering.md §6.2](../../../../requirements/deprecated/m1-run-timing-and-event-triggering.md), R21).
