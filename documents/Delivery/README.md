...
| [testing-guide.md](testing-guide.md) | Human procedure | The two test paths, and the screen/log/pcap evidence a run must produce |

Read them in that order — the testing guide assumes an installed app. Each numbers its own steps from 1, so a step number is only ever a step of the document naming it.
