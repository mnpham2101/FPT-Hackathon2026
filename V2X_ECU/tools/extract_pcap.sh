...
#
# HOST TOOL ONLY - never shipped in the v2x-ecu image (V2X_ECU/tools/ is
# excluded from the runtime stage). Runs in Git Bash on Windows and on Linux;
# bash + coreutils `base64` are the only dependencies.
#
# INPUT FORMAT - frozen by the producer V2X_ECU/capture.sh (see its "EXPORT
# BLOCK FORMAT" header):
#
#     [PCAP-BEGIN <basename>]
...
View Log into a .pcap file, ready for Wireshark.
  ...
  <saved-view-log>  log file saved from the V2X ECU node's View Log ("-" is not
