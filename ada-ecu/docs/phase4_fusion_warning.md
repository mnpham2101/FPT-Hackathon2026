...
| 25 m | C enters gate, R4 `riskState = warning` |
| 24 m | C stays in gate, no duplicate R4 |
| 36 m | C exits gate, R4 `riskState = clear` |

## Timeout Clear Smoke

This emits one warning when C enters the gate, then emits one `clear` after `miss_limit_ms` because no more R2 messages arrive:
