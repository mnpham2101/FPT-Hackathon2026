...
- **Ego Tx: broadcast own perception (R10).** The V2X ECU constructs CPMs from the ADA store snapshot and broadcasts them via the R7 adapter `send`, so ego relays what *it* perceives onward to a following vehicle. Deferred from M1 (2026-07-30): M1's V2X data path is receive-only — ego is the warned vehicle A, never the relaying vehicle B, and B is simulated by the bench (R11). The `send` seam already exists, so this returns as an implementation, not a redesign.
- **Commands to other ECUs.** ADA's output stage extends to command/actuation output to further ECUs/hardware; M1 implements no ADA→V2X output at all (the R10 store snapshot is deferred above).
