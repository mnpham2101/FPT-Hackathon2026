...
- **Ego Tx: quảng bá nhận thức của chính mình (R10).** V2X ECU dựng CPM từ snapshot kho lưu trữ của ADA và quảng bá qua `send` của seam R7, để ego chuyển tiếp những gì *chính nó* nhận thức được tới xe phía sau. Hoãn khỏi M1 (2026-07-30): đường dữ liệu V2X của M1 chỉ nhận — ego là xe được cảnh báo A, không bao giờ là xe chuyển tiếp B, và B do bench mô phỏng (R11). Seam `send` đã có sẵn, nên tính năng này quay lại như một phần hiện thực, không phải thiết kế lại.
- **Lệnh tới các ECU khác.** Tầng đầu ra của ADA mở rộng thành đầu ra lệnh/chấp hành tới các ECU/phần cứng khác; M1 không hiện thực đầu ra ADA→V2X nào (snapshot lưu trữ R10 đã hoãn ở mục trên).
