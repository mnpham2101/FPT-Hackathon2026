...
| ~~List deployments~~ | ~~`GET /api/v1/deployments`~~ | **not live** — 404 `Route not found` (2026-07-31). Obtain `roomId` from the deploy response, the Nydus UI, or a sidecar log's `connecting to room <roomId>` line. Deploying also creates a snapshot blueprint `<name>-deploy`; edit the original and redeploy |
