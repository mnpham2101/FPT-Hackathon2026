...
- [video-source-for-r12.md](../ADA_ECU/doc/research_notes/video-source-for-r12.md) — the clip baked into the image with no external arming path; `VIDEO_CLIP_PATH` and `DETECTOR_FRAME_STRIDE`; the 5 Hz sample-rate budget.
- [ivi-ecu-hld.md](../IVI_ECU/doc/ivi-ecu-hld.md) and its [decisions](../IVI_ECU/doc/ivi-ecu-design-decisions.md) · `R4Message.kt` · `build.gradle.kts` — R4 carries no timestamp; `DisplayMode` Home/Apps/Settings/Warning and the `[UI]` line with its `cause`; `WARNING_TIMEOUT_MS` = 10000 as a local countdown, with active risk raising the warning and only silence lowering it; R3 timestamps parsed and unused.
