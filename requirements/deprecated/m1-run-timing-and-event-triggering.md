...
|---|---|---|
| Bench tick → CPM bytes (persistent `cpm_encode --stream` subprocess, not a fork per message) | unmeasured; pipe round-trip to a resident process | [SP D1](../../documents/Design/MODULE-DESIGN/SCENARIO-PLAYER/scenario-player-design-decisions.md) |
...
- [phase0-smoke-test-run.md](../../plans/doc/deprecated/phase0-smoke-test-run.md) · [baseline-connectivity-smoke-test.md](../../documents/Delivery/Test-Guides/baseline-connectivity-smoke-test.md) · [netcheck.py](../../tools/netcheck/netcheck.py) — the smoke test is a one-way relay chain, not a handshake; `START_DELAY_S` is its only readiness mechanism; hop 3 unconfirmed (O4).
...
- [scenario-player-hld.md](../../documents/Design/MODULE-DESIGN/SCENARIO-PLAYER/scenario-player-hld.md) · [generator.py](../../Scenario_Player/player/generator.py) · [config.py](../../Scenario_Player/player/config.py) · [default.yaml](../../Scenario_Player/scenarios/default.yaml) — the tick-counter scenario clock, the fixed-sleep loop, the YAML key set, the persistent `cpm_encode --stream` codec path (D1).
- [ada-ecu-hld.md](../../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-hld.md) and its [decisions](../../documents/Design/MODULE-DESIGN/ADA-ECU/ada-ecu-design-decisions.md) — `TRACK_TIMEOUT_MS` as silence and `DETECTOR_FRAME_STRIDE` as decimation rather than a rate (the unstated clock source that §6.2 settles); D3's one admission machine for both sources with `CONFIRM_HITS` and the `GATE_ENTER_M`/`GATE_EXIT_M` Schmitt band; D5's risk bands on the composed range `d_AC`, its `RISK_DWELL_MS` debounce, its edge-triggered emission in both directions and its `b_unknown` path; D6's frame-source seam and range estimator; the env table's committed threshold values.
...
- [ivi-ecu-hld.md](../../documents/Design/MODULE-DESIGN/IVI-ECU/ivi-ecu-hld.md) and its [decisions](../../documents/Design/MODULE-DESIGN/IVI-ECU/ivi-ecu-design-decisions.md) · `R4Message.kt` · `build.gradle.kts` — R4 carries no timestamp; `DisplayMode` Home/Apps/Settings/Warning and the `[UI]` line with its `cause`; `WARNING_TIMEOUT_MS` = 10000 as a local countdown, with active risk raising the warning and only silence lowering it; R3 timestamps parsed and unused.
