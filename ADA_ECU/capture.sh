#!/bin/sh
# capture.sh - ADA ECU in-container ADA->IVI traffic capture (HLD D9, R6/R15/R19
# evidence). The ADA copy of the proven V2X_ECU/capture.sh - duplicated per
# folder, not shared, so the build context stays self-contained.
#
# Started in the BACKGROUND by entrypoint.sh (`./capture.sh &`) alongside the
# foreground ada_ecu app: this script must never take the container down, so it
# never exits non-zero and degrades instead of dying.
#
# Two tcpdump processes, two consumers (HLD D9):
...
#
# EXPORT BLOCK FORMAT - frozen contract, parsed by ADA_ECU/tools/extract_pcap.sh
# (subtask 6.4.4.2) and documented in requirements/car-sky-guide/traffic-capture-wireshark.md:
#
#     [PCAP-BEGIN <basename>]
...
#
# DEGRADED MODES (fallback posture - log and stay alive, never exit):
...
# Fallback: no capture handle, so report interface packet counters forever.
counter_fallback() {
...
# multi-word BPF filter (e.g. "udp port 47300") splits into tcpdump arguments.
