...
| 2 | **Closed in implementation:** the tracked lowercase `ada-ecu/` folder has been removed and the runnable ADA Phase 2/3/4 code now lives under canonical `ADA_ECU/`. | Phase 2/3/4 |
...
| 7 | **Closed in documentation:** `presentation/ada/ada-phase2-3-4-deck.*` and `ADA_ECU/docs/phase2_3_4_test_demo_guide.md` now point to canonical `ADA_ECU/` commands. | Phase 2/3/4 |
| 8 | **Closed for skeleton demo:** `media/ego-b-occluding-c.mp4` is present and `tools/smoke_demo_video_detector.py` proves video decode → R3 object emission. The current backend is still `placeholder`; YOLO/ONNX remains the next detector upgrade if required for final R12 evidence. | Phase 3 |
