...
>
> **Location note:** this HLD is cross-cutting (all four nodes), so it lives in `plans/doc/` per [CLAUDE.md § Repository layout](../CLAUDE.md); later node-scoped HLDs (R11 bench, Phase 2 ADA) land in their own node's `doc/`.
>
...
- **Why no sanctioned location fits:** these are machine-consumed artifacts shared by all four nodes — `doc/` subfolders are documentation-only ([CLAUDE.md § Repository layout](../CLAUDE.md)), no single node folder may own them (self-contained folders, no cross-node reads), and `plans/` holds plans. A shared artifact needed by two nodes is a *contract deliverable* ([CLAUDE.md § Repository layout](../CLAUDE.md)) — this folder is where contract deliverables live.
...
| 1 | **Bench Python → R1 codec path** (binding, helper subprocess, or pre-encoded vectors) — standing open item in [CLAUDE.md § Repository layout](../CLAUDE.md); candidates ranked in callflow note F3. Restated open — **decided in the R11 HLD, not here.** | project-architecture, R11 HLD |
