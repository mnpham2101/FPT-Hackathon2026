...
loop). Startup is announced with one ``[START]`` line and any startup/fatal exception is logged as
one ``[FATAL]`` line to stdout, both flushed so they land in the container View Log; the process
exit code is 0 on a clean run, 1 on a fatal error. Stdlib + the ``player`` package only.
"""
    ...
    *,
    ...
    try:
        ...
        with (
            ...
            Generator(scenario, scenario_cfg, encoder.encode, sender.send, log=sink).run(
                max_ticks=max_ticks
            )
        return 0
