...
| 2 | **Partly resolved — library skew closed, arm64 build proof pending.** Builder and runtime stages share one base image (`python:3.11-slim`, §4 runtime; fallback F1 ratified by the architecture owner 2026-08-01), with the CMake ≥3.28 floor met by a pip `cmake` wheel instead of apt, so `cpm_encode` links against the glibc/libstdc++ it runs on and no stage-1 → stage-2 mismatch is possible; the report §4 LGPLv3 dynamic posture is satisfied by staging `libvanetza_asn1*.so` into the runtime stage. Implemented in [Dockerfile](../Dockerfile). Still open: single-platform `linux/arm64` (`--platform`, smoke-test note M3) is unproven until the image lane builds it. | skew: closed · arm64: `5.1.8.2` lane |
