...
A node's component diagram and its call flow already exist in that node's `doc/` — the HLD is where the design was decided, so it is where the deck's picture of it comes from.
