...
- Every subtask brief names paths inside the owning node folder per [node-code-layout.md](../rules/node-code-layout.md) (`Scenario_Player/` = R11 bench, `V2X_ECU/` = R1/R7–R9 (R10 deferred), `ADA_ECU/` = R3/R12–R15, `IVI_ECU/` = R4/R16–R17) and states that node's build command as part of the subtask's build-passes criterion.
...
- **For deployment status, platform build/test, or acceptance evidence, spawn [[car-sky]]** rather than checking it here — it runs [carsky-acceptance-evidence](../skills/carsky-acceptance-evidence/SKILL.md) (CI config → commit/push gates → build verification → registry check → deployment status) and returns evidence per criterion. The planner keeps the task ID and the done-marking; [[car-sky]] only supplies the evidence.
...
- **Mark a task done** only when its subagent has both (a) met the subtask's single objective per its acceptance criteria, build, and tests, and (b) made the atomic commit. Where an acceptance criterion depends on something running on CarSky, the evidence comes from [[car-sky]] per the skill above.
