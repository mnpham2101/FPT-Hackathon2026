...
- Every subtask brief names paths inside the owning node folder, and states that node's build command as part of the subtask's build-passes criterion. The folder → node → requirement map and the build rules are in [CLAUDE.md § Repository layout](../../CLAUDE.md).
...
- Phase content (objectives, tasks, tech stack, acceptance criteria) comes from the active plan doc, currently [milestone1_high_level_plan.md](../../documents/Plan/milestone1_high_level_plan.md), plus [[project-researcher]]'s enumerated requirements and [[project-architecture]]'s design/contracts.
- **When the objective is test, verification or deployment** — and only then — read that phase's node reference(s) under `requirements/car-sky-guide/` plus the delivery guides in [documents/Delivery/](../../documents/Delivery/), and decompose the subtasks from them. Cite the section that governs each step rather than copying its commands into the brief. A phase with no such work leaves those files unread.
...
- `requirements/car-sky-guide/` node references and the delivery guides in [documents/Delivery/](../../documents/Delivery/), read only when the objective is test, verification or deployment — those subtasks are decomposed from them.
- Current state of the codebase (must read directly).
- The active plan doc, [milestone1_high_level_plan.md](../../documents/Plan/milestone1_high_level_plan.md), and [task-planning-conventions.md](../rules/task-planning-conventions.md).
