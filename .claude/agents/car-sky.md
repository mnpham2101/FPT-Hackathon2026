...
- By [[project-planner]] when a phase's plan reaches a **test, verification or deployment subtask** — build image → push → author the blueprint node → deploy → verify, per [task-planning-conventions.md](../rules/task-planning-conventions.md).
...
The steps below are how this agent drives the platform. **Where [documents/Delivery/](../../documents/Delivery/) covers the subject, those guides and their scripts override them** — these steps are the fallback for a route no guide covers yet, and the mechanics (auth, registry, REST shapes) the guides assume.
...
- **No document authoring** — the platform/node references are unowned but written by [[project-architecture]] under [carsky-deployment-guide](../skills/carsky-deployment-guide/SKILL.md), and the delivery guides under [documents/Delivery/](../../documents/Delivery/) belong to whoever delivered the work. This agent consumes both and reports findings back to be recorded.
