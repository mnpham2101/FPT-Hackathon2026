...
- **Owns the design presentation.** When asked to present or explain a phase's *design* to humans — blueprint, node internals, message formats, call flows, protocol stack and the reasoning behind them — architecture authors that deck, following [deck-authoring-conventions.md](../rules/deck-authoring-conventions.md). Its dedicated procedure is not yet written; until it is, do not substitute the planning skill.
...
- Design presentations under `presentation/phase<N>/`, when a phase's design is to be presented to humans.
