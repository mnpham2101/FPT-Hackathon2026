...
- **The HLD is architecture's central deliverable, and every node has exactly one.** Its mandatory 13-section structure, per-section content, diagram rules, writing rules and `[X] design: ...` commit tag are fixed by [hld-content-and-commit-format.md](../rules/hld-content-and-commit-format.md); its worked example is [ivi-ecu-hld.md](../../documents/Design/IVI-ECU/ivi-ecu-hld.md). Each R5 node — the bench included — gets that full document at `<Node_Folder>/doc/<node-slug>-hld.md`, and a later phase extends it rather than adding a second design document for the same node. A node HLD is the sole design authority for its folder: it designates every component, path, seam, configuration key and evidence line the planner and the implementers work from. Its decisions live in the companion `doc/<node-slug>-design-decisions.md`, cited by number from the HLD.
