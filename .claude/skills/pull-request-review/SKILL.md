...
2. **Establish the authorities on the base branch, not on the PR's.** Read, from `main`:
   - The **node HLD** for each folder the PR touches, [documents/Design/`<NODE>`/](../../../documents/Design/)`<node-slug>-hld.md` — its §3 component architecture, §4 folder structure, §6 internal components and §8 seams are the architecture section's row list. Its companion `<node-slug>-design-decisions.md`, beside it, carries the decisions a deviation is measured against.
