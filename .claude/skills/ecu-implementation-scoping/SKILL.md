...
1. **Resolve phase/requirement → node(s).** Cross-reference the requested phase in [milestone1_high_level_plan.md](../../../documents/Plan-Proposal/milestone1_high_level_plan.md) §5 and the requirement's home node in the report ([m1-cooperative-awareness.md](../../../requirements/m1-cooperative-awareness.md) §2) against the folder → node → requirement map in [node-code-layout.md](../../rules/node-code-layout.md), which also fixes each node's language and build artifact.
