...
4. **Source existing research notes.** Check `doc/research_notes/` in `plans/` and in each resolved node folder — the per-folder `doc/` locations defined in [node-code-layout.md §Per-folder doc/](../../rules/node-code-layout.md#per-folder-doc) — for notes already covering part of the feature (e.g. [baseline-connectivity-smoke-test.md](../../../plans/doc/research_notes/baseline-connectivity-smoke-test.md) for the Phase 0 smoke test). Reference them from the HLD — link plus a one-line statement of what is adopted — instead of re-deriving or restating their content. Notes are non-authoritative scratch: on any conflict, the CLAUDE.md document-authority order wins.
