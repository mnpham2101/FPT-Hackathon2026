...
4. **Source existing research notes.** Check [documents/KnowledgeBase/](../../../documents/KnowledgeBase/), the resolved node's folder under [documents/Design/](../../../documents/Design/), and `plans/doc/research_notes/` — the locations defined in [CLAUDE.md § Repository layout](../../../CLAUDE.md) — for notes already covering part of the feature (e.g. [baseline-connectivity-smoke-test.md](../../../documents/Delivery/baseline-connectivity-smoke-test.md) for the Phase 0 smoke test). Reference them from the HLD — link plus a one-line statement of what is adopted — instead of re-deriving or restating their content. Notes are non-authoritative scratch: on any conflict, the CLAUDE.md document-authority order wins.
