...
- **Environment (M1):** `https://hackathon-2.carsky.io` — the base URL. Environment-specific; confirm it hasn't changed (the org may rotate hosts between rounds).
- **Auth:** REST calls use header `Authorization: Bearer <API_KEY>`. The key is minted in the UI: **Settings (⚙) → Credentials → New credential** (shown once). It is an OIDC machine-to-machine credential — **not** a Keycloak user password, and not derivable from one.
...
- **Full REST reference + verified request shapes:** [carsky-rest-api-blueprint.md](../../../requirements/car-sky-guide/carsky-rest-api-blueprint.md) — endpoints, the atomic `/batch` node-create payload, `validate`, and the OpenAPI spec at `GET /api/v1/openapi.json`.
