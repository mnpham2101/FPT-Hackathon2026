...
- **Exception — subtask `**Status:**` lines** in the phase plans under [plans/](../../../plans/): these carry the auditable proof of work a subtask closed with — completion date, commit hash, CI run number, and any deviation from the brief. That's engineering traceability, not investigation narrative, and stays exempt from the no-date/no-history rule above. Still write it as a conclusion (what shipped, what it proves) — evidence citations, not a discovery story.

## Audience: pick the format for who reads it

- **Agent-context reference** (default — research notes, rule docs, HLDs, per-node guides, API references): rules 1–6 as-is. Dense, scannable, no prose padding. A **node HLD** additionally follows [hld-content-and-commit-format.md](../../rules/hld-content-and-commit-format.md) — the mandatory section order, the companion decision record, and its writing rules (one claim per sentence, facts in tables, each fact stated once); its worked example, [ivi-ecu-hld.md](../../../documents/Design/IVI-ECU/ivi-ecu-hld.md), is the style to match.
- **Human how-to guide** (a walkthrough read once and followed): simple language, numbered steps, one action per step. Still no hard line-wrap, still no invented terms. An operational walkthrough under [requirements/car-sky-guide/](../../../requirements/car-sky-guide/) — the authoritative build/deploy/install/verify procedures — additionally follows [walkthrough-authoring](../walkthrough-authoring/SKILL.md): read it before authoring or editing one.
- **Human-facing presentation** (meant to be read or exported as slides): author as a Marp deck — `marp: true` frontmatter, `---` slide separators, `<!-- _class: lead -->` section-divider slides, one idea per slide. Follow [m1-proposal-deck.md](../../../presentation/m1-proposal-deck.md) as the template. Write only the markdown source: the HTML export is generated by [build-slides.py](../../../presentation/slide-build-tool/build-slides.py) from [template.md](../../../presentation/template/template.md), so the design isn't re-derived from scratch each time. Read [deck-authoring-conventions.md](../../rules/deck-authoring-conventions.md) first — file placement, the asset policy, and the build workflow.
