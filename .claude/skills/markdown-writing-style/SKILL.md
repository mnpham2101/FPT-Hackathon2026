...
- **Agent-context reference** (default — research notes, rule docs, HLDs, per-node guides, API references): rules 1–6 as-is. Dense, scannable, no prose padding. A **node HLD** additionally follows [hld-content-and-commit-format.md](../../rules/hld-content-and-commit-format.md) — the mandatory section order, the companion decision record, and its writing rules (one claim per sentence, facts in tables, each fact stated once); its worked example, [ivi-ecu-hld.md](../../../IVI_ECU/doc/ivi-ecu-hld.md), is the style to match.
