...
- **Agent-context reference** (default — research notes, rule docs, HLDs, per-node guides, API references): rules 1–6 as-is. Dense, scannable, no prose padding. A **node HLD** additionally follows [hld-content-and-commit-format.md](../../rules/hld-content-and-commit-format.md) — the mandatory section order, the companion decision record, and its writing rules (one claim per sentence, facts in tables, each fact stated once); its worked example, [ivi-ecu-hld.md](../../../documents/Design/MODULE-DESIGN/IVI-ECU/ivi-ecu-hld.md), is the style to match.
- **Human how-to guide** (read once and followed): simple language, numbered steps, one action per step, the expected result stated under each command. Still no hard line-wrap, still no invented terms. The build/deploy/install/verify procedures live in [documents/Delivery/](../../../documents/Delivery/) beside the scripts that automate them; follow the shape of the guides already there.
