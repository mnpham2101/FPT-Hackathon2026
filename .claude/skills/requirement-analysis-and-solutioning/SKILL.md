...
1. **Analyse feasibility.** Before decomposing anything, sanity-check whether the input is realistically achievable at all — against the milestone timeline, the active plan's scope/assumptions ([milestone1_high_level_plan.md](../../../documents/Plan-Proposal/milestone1_high_level_plan.md) sections 2 and 6), and the hard constraints in [solution-selection-criteria.md](../../rules/solution-selection-criteria.md) (open-source only, Linux-targeted). Flag — don't silently drop — anything that looks infeasible as stated, with reasoning.
