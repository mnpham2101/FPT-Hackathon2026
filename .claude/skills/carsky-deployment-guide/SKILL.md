...
**Scope boundary — reference, not procedure.** This skill produces the files that state what the platform and each node *are*: `node-*.md`, the blueprint and REST references. The procedure a human *follows* end to end — build → export → deploy → install → verify — lives in [documents/Delivery/](../../../documents/Delivery/) beside the scripts that automate it. When a request asks for that, write it there rather than a second procedure here; the two would drift.
...
Owned by [[project-architecture]]. These files are what a delivery guide links to for facts, and what [[project-planner]] reads when a phase actually has deployment work. Produces no task IDs, no procedures, and spawns no implementation.
