...
Builds the outgoing delivery package — `Hackathon-Round2-Delivery/` and `Hackathon-Round2-Delivery.zip` inside this folder, both gitignored. One command, from the repo root:
...
6. **Verifies** — landing-page targets exist, no internal/code/markdown paths ship, exactly one video copy, and every relative link in every shipped HTML page resolves — then **zips** the result to `Hackathon-Round2-Delivery.zip`, the file to send out.
...
Edit it here, then rerun the build — never edit `Hackathon-Round2-Delivery/index.html`, which every build overwrites.
